        ...
        <div class="container mx-auto px-6 md:px-8">
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-8">
                <div class="lg:col-span-2">
                    <form method="POST" action="{{ route('checkout.process') }}" enctype="multipart/form-data" class="space-y-6 bg-white p-6 rounded-2xl shadow-sm border border-gray-100" x-data="{ 
                        notes: '{{ old('notes') }}'
                    }">
                        @csrf
                        ...
                        @if($hasPrescriptionDrugs)
                            <div class="p-6 bg-rose-50 border border-rose-100 rounded-2xl space-y-4">
                                <div class="flex items-start gap-3">
                                    ...
                                    <div>
                                        <h3 class="text-lg font-bold text-rose-900">Upload Resep Dokter</h3>
                                        <p class="text-sm text-rose-700">Pesanan Anda mengandung <strong>Obat Keras</strong>. Berdasarkan regulasi BPOM, Anda wajib melampirkan resep fisik asli.</p>
                                    </div>
                                </div>

                                <div>
                                    <label class="block text-sm font-semibold text-rose-800 mb-2">Pilih File Resep (JPG, PNG, PDF - Max 5MB)</label>
                                    <input type="file" name="prescription_file" 
                                           class="w-full bg-white rounded-xl border border-rose-200 px-4 py-3 focus:ring-2 focus:ring-primary focus:border-transparent"
                                           accept=".jpg,.jpeg,.png,.pdf" required>
                                    @error('prescription_file')<p class="text-rose-600 text-sm mt-1">{{ $message }}</p>@enderror
                                </div>

                                <div class="flex flex-col md:flex-row gap-3 pt-2">
                                    <a href="{{ route('prescriptions.index') }}" target="_blank" class="flex-1 px-4 py-2 bg-white text-rose-600 border border-rose-200 rounded-xl text-xs font-bold tracking-widest uppercase hover:bg-rose-50 transition-colors text-center">
                                        Gunakan Fitur Upload Resep
                                    </a>
                                    <button type="button" id="remove-restricted-btn" class="flex-1 px-4 py-2 bg-gray-900 text-white rounded-xl text-xs font-bold tracking-widest uppercase hover:bg-primary transition-colors text-center">
                                        Singkirkan Obat Keras
                                    </button>
                                </div>
                            </div>
                        @endif
                        ...
                        <div class="space-y-3 mb-4 max-h-64 overflow-y-auto">
                            @if(isset($selectedItems) && $selectedItems->count() > 0)
                                @foreach($selectedItems as $item)
                                    ...
                                    @endphp
                                    <div class="flex items-center gap-3 p-3 bg-gray-50 rounded-xl {{ $isMedicine && $item->medicine?->classification === 'Obat Keras' ? 'border-l-4 border-rose-500' : '' }}">
                                        ...
                                        <div class="flex-1 min-w-0">
                                            <div class="flex items-center gap-1.5 mb-0.5">
                                                ...
                                                @if($isMedicine && $item->medicine?->classification === 'Obat Keras')
                                                    <span class="flex-shrink-0 w-2 h-2 rounded-full bg-rose-500" title="Obat Keras"></span>
                                                @endif
                                            </div>
...
@section('scripts')
    <script>
        document.addEventListener('DOMContentLoaded', () => {
            ...
            const removeRestrictedBtn = document.getElementById('remove-restricted-btn');
            if (removeRestrictedBtn) {
                removeRestrictedBtn.addEventListener('click', () => {
                    const selectedItems = @json($selectedItems->values()->map(fn($item) => [
                        'id' => $item->id,
                        'is_keras' => $item->medicine_id && $item->medicine?->classification === 'Obat Keras'
                    ]));
                    
                    const newSelectedIds = selectedItems
                        .filter(item => !item.is_keras)
                        .map(item => item.id);
                    ...
                    url.searchParams.set('selected_items', JSON.stringify(newSelectedIds));
